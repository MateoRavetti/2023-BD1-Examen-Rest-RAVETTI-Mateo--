...
from .models import Products, Categories, Orderdetails
...
from rest_framework.decorators import api_view
from rest_framework.response import Response
from .models import Employees, Orders, Orderdetails, Products, Categories
...
def get_employees(request):
    ...
    try:
        ...
        category = Categories.objects.get(categoryid=category_id)
...
def update_products(request):
    ...
    ventas_requeridas = request.data.get('ventasRequeridas')
    aumento = request.data.get('aumento')
    shipper_id = request.data.get('ShipperID')

    try:
        
        category = Categories.objects.get(CategoryID=category_id)
        shipper = Shippers.objects.get(ShipperID=shipper_id)
    except (Categories.DoesNotExist, Shippers.DoesNotExist):
        return Response({"error": "Categoría o Shipper no encontrados"}, status=404)
    ...
    if products.exists():
        for product in products:
            ...
            if shipper.companyname.startswith('United'):
        ...
        serializer = ProductUpdateSerializer(products, many=True)
        return Response(serializer.data, status=200)
    else:
        return Response({}, status=204)
